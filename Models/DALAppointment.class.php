...
class DALAppointment extends DALConnectionToDB{
   
   public function read($id_appointment=NULL){

      if(empty($id_appointment)){
         $this->maRequete = 'SELECT * from appointments';
      } else {
   ...
   public function update($id_appointment, $last_name, $first_name, $email, $password, $address, $phone, $date, $hour) {



  $this->maRequete = "UPDATE `appointments` SET `id_appointment` = '$id_appointment', `last_name` = '$last_name', `first_name` = '$first_name', `email` = '$email', `password` = '$password', `address` = '$address', `phone` = '$phone', `date` = '$date' , `hour` = '$hour'  WHERE `appointments`.`id_appointment` = '$id_appointment'";
...
//  header("location: ../Controllers/Calendar_C.php");
   ...
   public function add($last_name, $first_name, $email, $password, $address, $phone, $date, $hour){

   $this->maRequete = "INSERT INTO appointments (`id_appointment`, `last_name`, `first_name`, `email`, `password`, `address`, `phone`, `date`, `hour`) VALUES(NULL, '$last_name', '$first_name', '$email', '$password', '$address', '$phone', '$date', '$hour')";
